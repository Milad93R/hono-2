...
class Logger {
  ...
  async info(message: string, ...args: any[]) {
    ...
    if (this.telegramService) {
      this.telegramService.logtelFormatted('INFO', fullMessage)
  ...
  async warn(message: string, ...args: any[]) {
    ...
    if (this.telegramService) {
      this.telegramService.logtelFormatted('WARN', fullMessage)
        .catch(err => console.error('Failed to send Telegram log:', err))
    }
  }
   ...
   */
  async error(message: string, error?: Error | any, ...args: any[]) {
    ...
    if (this.telegramService) {
      this.telegramService.logtelFormatted('ERROR', fullMessage)
        .catch(err => console.error('Failed to send Telegram log:', err))
    }
  }
